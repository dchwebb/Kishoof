...
void USB::InterruptHandler()					// In Drivers\STM32H7xx_HAL_Driver\Src\stm32h7xx_hal_pcd.c
{
	...
	if ((USB_OTG_HS->GINTSTS & USB_OTG_HS->GINTMSK) == 0) {
		return;
	}
	...
	if (ReadInterrupts(USB_OTG_GINTSTS_RXFLVL))	{
		...
		const uint32_t receiveStatus = USB_OTG_HS->GRXSTSP;		// OTG status read and pop register: not shown in SFR, but read only (ie do not pop) register under OTG_FS_GLOBAL->FS_GRXSTSR_Device
	...
	if (ReadInterrupts(USB_OTG_GINTSTS_IEPINT)) {
		...
		while (ep_intr != 0) {
			if ((ep_intr & 1) != 0) {
				...
				epint = USBx_INEP(epnum)->DIEPINT & (USBx_DEVICE->DIEPMSK | (((USBx_DEVICE->DIEPEMPMSK >> (epnum & epAddressMask)) & 0x1U) << 7));

				USBUpdateDbg(epint, {}, epnum, {}, nullptr);

				if ((epint & USB_OTG_DIEPINT_XFRC) == USB_OTG_DIEPINT_XFRC) {			// 0x1 Transfer completed interrupt
					uint32_t fifoemptymsk = (0x1UL << (epnum & epAddressMask));
					...
					if (epnum == 0) {
						...
						classByEP[epnum]->transmitting = false;
					}
				}
				...
				if ((epint & USB_OTG_DIEPINT_TXFE) == USB_OTG_DIEPINT_TXFE) {			// 0x80 Transmit FIFO empty
					...
					if (epnum == 0) {
						...
						if (classByEP[epnum]->inBuffSize <= classByEP[epnum]->inBuffCount) {
							const uint32_t fifoemptymsk = (0x1UL << (epnum & epAddressMask));
							USBx_DEVICE->DIEPEMPMSK &= ~fifoemptymsk;
						}
					}
				}
	...
	if (ReadInterrupts(USB_OTG_GINTSTS_SRQINT))	{
		USB_OTG_HS->GINTSTS &= USB_OTG_GINTSTS_SRQINT;
	}
	...
	if (ReadInterrupts(USB_OTG_GINTSTS_OTGINT)) {
		uint32_t temp = USB_OTG_HS->GOTGINT;
...
void USB::Init(bool softReset)
{
	...
	USB_OTG_HS->GAHBCFG &= ~USB_OTG_GAHBCFG_GINT;		// Disable the controller's Global Int in the AHB Config reg
	...
	// Enable interrupts
	USB_OTG_HS->GINTMSK = 0U;							// Disable all interrupts
	USB_OTG_HS->GINTMSK |= USB_OTG_GINTMSK_RXFLVLM | USB_OTG_GINTMSK_USBSUSPM |			// Receive FIFO non-empty mask; USB suspend
	...
	USB_OTG_HS->GRXFSIZ = 128;		 									// Rx FIFO depth
	...
	USB_OTG_HS->DIEPTXF0_HNPTXFSIZ = (64 << USB_OTG_TX0FD_Pos) |		// IN Endpoint 0 Tx FIFO depth
	...
	USB_OTG_HS->DIEPTXF[0] = (64 << USB_OTG_DIEPTXF_INEPTXFD_Pos) |		// IN endpoint 1 Tx FIFO depth
	...
	USB_OTG_HS->DIEPTXF[1] = (64 << USB_OTG_DIEPTXF_INEPTXFD_Pos) |		// IN endpoint 2 Tx FIFO depth
...
{
#if (USB_DEBUG)
	if (((USB_OTG_HS->GINTSTS & USB_OTG_HS->GINTMSK) & interrupt) == interrupt) {
		usbDebug[usbDebugEvent & usbDebugMask].eventNo = usbDebugEvent;
		usbDebug[usbDebugEvent & usbDebugMask].Interrupt = USB_OTG_HS->GINTSTS & USB_OTG_HS->GINTMSK;
		usbDebugEvent++;
	}
#endif

	return ((USB_OTG_HS->GINTSTS & USB_OTG_HS->GINTMSK) & interrupt) == interrupt;
}
...
size_t USB::SendData(const uint8_t* data, const uint16_t len, uint8_t endpoint)
{
	...
	if (devState == DeviceState::Configured && !classByEP[endpoint]->transmitting) {
		classByEP[endpoint]->transmitting = true;
...
{
	uint16_t counter = 0;
	while (cdc.transmitting && counter < 10000) {
		++counter;
	}
...
{
	uart.SendString("Event,Interrupt,Desc,Int Data,Desc,Endpoint,mRequest,Request,Value,Index,Length,PacketSize,XferBuff\n");
	std::string interrupt, subtype;

	for (uint32_t i = 0; i < usbDebugCount; ++i) {

		const uint16_t evNo = (usbDebugEvent + i) & usbDebugMask;
