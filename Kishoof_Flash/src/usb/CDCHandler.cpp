...
void CDCHandler::ProcessCommand()
{
	...
	if (cmd.compare("help") == 0) {
		...
		while (wavetable.sampleList[pos].name[0] != 0) {
			printf("%3lu %.11s %7lu %7lu %3u%1s %8u %s     0x%08x %.3f  \r\n",
					pos,
					wavetable.sampleList[pos].name,
					wavetable.sampleList[pos].size,
					wavetable.sampleList[pos].sampleRate,
					wavetable.sampleList[pos].byteDepth * 8,
					wavetable.sampleList[pos].dataFormat == 3 ? "f" : " ",		// floating point format
					wavetable.sampleList[pos].channels,
					wavetable.sampleList[pos].valid ? "Y" : " ",
					(unsigned int)wavetable.sampleList[pos].startAddr,
					(float)wavetable.sampleList[pos].sampleCount / wavetable.sampleList[pos].sampleRate
